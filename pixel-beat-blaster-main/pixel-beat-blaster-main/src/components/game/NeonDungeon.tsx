...
export function NeonDungeon() {
  ...
  const enemyCanOccupy = (r: Room, px: number, py: number, radius: number, flying = false) => {
    if (px - radius < TILE || px + radius > W - TILE || py - radius < TILE || py + radius > H - TILE) {
      return false;
    }
    const offsets = [
      { x: 0, y: 0 },
    ...
    for (const off of offsets) {
      if (solidFor(r, px + off.x, py + off.y, flying)) return false;
    }
    return true;
  };

  const hasRaycastWall = (r: Room, x1: number, y1: number, x2: number, y2: number) => {
    const dist = Math.hypot(x2 - x1, y2 - y1);
    const steps = Math.ceil(dist / (TILE / 2));
    for (let i = 1; i <= steps; i++) {
      const px = x1 + ((x2 - x1) * i) / steps;
      const py = y1 + ((y2 - y1) * i) / steps;
      if (blocksShot(r, px, py)) return true;
    }
    return false;
  };

  const spawnPickupOnWalkableTile = useCallback(
    (r: Room, prefX: number, prefY: number, kind: "block" | "coin", track?: TrackId, color: string = "#ffe23d") => {
      let targetX = Math.min(Math.max(TILE * 1.5, prefX), W - TILE * 1.5);
      let targetY = Math.min(Math.max(TILE * 1.5, prefY), H - TILE * 1.5);

      const isWalkable = (x: number, y: number) => {
        const id = tileAt(r, x, y);
        return id === T.FLOOR || id === T.BPM_UP || id === T.BPM_DOWN || id === T.AMPLIFIER;
      };
        ...
        for (let radius = 1; radius <= 6; radius++) {
          for (let dy = -radius; dy <= radius; dy++) {
            for (let dx = -radius; dx <= radius; dx++) {
              const testX = targetX + dx * TILE;
              const testY = targetY + dy * TILE;
  ...
  const clearRoom = useCallback(
    (r: Room) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const prefX = W / 2 + (i - (n - 1) / 2) * 46;
        const prefY = r.layout === "ISLAND" ? H / 2 - 120 : H / 2;
        spawnPickupOnWalkableTile(r, prefX, prefY, "block", track, t.color);
      }
      if (r.type === "BOSS") {
    ...
    for (const e of r.enemies) {
      ...
      if (e.behavior === "LASER_SNIPER_LOCK") {
        if (e.steps % 4 === 0) {
          ...
          if (!hasRaycastWall(r, e.x, e.y, target.x, target.y)) {
          ...
          if (!hasRaycastWall(r, e.x, e.y, player.current.x, player.current.y)) {
      ...
      if (e.behavior === "INFECTED_SPEAKER_SPAWNER" && e.steps % 4 === 0) {
        ...
        const MAX_SPEAKER_SPAWNS = 5;
        const currentSpawns = e.spawnsCount ?? 0;
        if (r.enemies.length < MAX_ROOM_ENEMIES && currentSpawns < MAX_SPEAKER_SPAWNS) {
          ...
          const eRadius = (def.id === "bass_dropper" ? 36 : 26) / 2;

          for (let attempts = 0; attempts < 8; attempts++) {
            const ang = (attempts / 8) * Math.PI * 2;
            const sx = e.x + Math.cos(ang) * 46;
            const sy = e.y + Math.sin(ang) * 46;

            if (enemyCanOccupy(r, sx, sy, eRadius, false)) {
              e.spawnsCount = currentSpawns + 1;
              r.enemies.push({
    ...
    const loop = (now: number) => {
      ...
      if (!frozen) {
        pickups.current = pickups.current.filter((p) => {
          ...
          // Efeito magnetico se o jogador estiver proximo (ate 70px)
          if (dist < 70 && dist > 0) {
            const pullSpeed = (70 - dist) * 4;
      ...
      if (!frozen) {
        ...
        for (const e of cur.enemies) {
          ...
          if (
            ...
            if (enemyCanOccupy(cur, nx, e.y, rRadius, flying)) e.x = nx;
            if (enemyCanOccupy(cur, e.x, ny, rRadius, flying)) e.y = ny;

            // Restrição estrita aos limites internos da sala
            e.x = Math.max(TILE + rRadius, Math.min(W - TILE - rRadius, e.x));
            e.y = Math.max(TILE + rRadius, Math.min(H - TILE - rRadius, e.y));
          }
          ...
          if (e.behavior === "SHOOT_AT_PLAYER_PERIODIC" || e.behavior === "BOSS_PATTERN_WAVES_AND_PROJECTILES") {
            ...
            if (e.cooldown <= 0) {
              ...
              } else if (!hasRaycastWall(cur, e.x, e.y, player.current.x, player.current.y)) {
            ...
            for (const e of cur.enemies) {
              ...
              if (Math.hypot(s.x - e.x, s.y - e.y) < e.size / 2 + s.r) {
                ...
                if (s.knock) {
                  ...
                  const rRadius = e.size / 2;
                  const flying = e.behavior === "FLY_IGNORES_CHASMS";
                  if (enemyCanOccupy(cur, kx, e.y, rRadius, flying)) e.x = kx;
                  if (enemyCanOccupy(cur, e.x, ky, rRadius, flying)) e.y = ky;
                  e.x = Math.max(TILE + rRadius, Math.min(W - TILE - rRadius, e.x));
                  e.y = Math.max(TILE + rRadius, Math.min(H - TILE - rRadius, e.y));
                }
                if (s.explode) {
        ...
        if (dying.length) {
          for (const e of dying) {
            ...
            for (let i = 0; i < n; i++) {
              const prefX = e.x + (Math.random() - 0.5) * 30;
              const prefY = e.y + (Math.random() - 0.5) * 30;
              spawnPickupOnWalkableTile(cur, prefX, prefY, "coin", undefined, "#ffe23d");
            }
          }
